...
part 1 do
  assert_call_on(Intcode.new("1002,4,3,4,33"), nil, :run)
  log_call_on(Intcode.new(input, input: 1), :run) # 9961446
end

part 2 do
  assert_call_on(Intcode.new("3,9,8,9,10,9,4,9,99,-1,8", input: 8), 1, :run)
  assert_call_on(Intcode.new("3,9,8,9,10,9,4,9,99,-1,8", input: 7), 0, :run)
  log_call_on(Intcode.new(input, input: 5, logging: true), :run) # 742621
end
